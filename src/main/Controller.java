...
public class Controller extends Application {
    ...
    /**
     * This inner class checks to see if the user can move on to the game screen. It checks to see if
     * the user entered a name that is not just spaces. Furthermore, the player must select
     * a name and a weapon.
     */
    class ProceedHandler implements EventHandler<ActionEvent> {
        ...
        public void handle(ActionEvent e) {
            try {
                ...
                if (!screen2.getNameEntry().getText().equals("") && screen2.getDifficulties().getSelectedToggle() != null
                    ...
                    if (((RadioButton) (screen2.getDifficulties().getSelectedToggle())).getText().equals("easy")) {
                        Player.setBalance(3000);
                        Player.setSpeed(10);
                        Player.setStrength(1200);
                        money = new Label("Money: $" + Integer.toString(Player.getBalance()));
                        money.setStyle("-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.3), 10,0.7,0.0,0.0); -fx-text-fill: yellow; -fx-font: 32pt 'Lao MN'");
                        health = new Label("Health: " + Integer.toString(Player.getHealth()));
                        health.setStyle("-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.3), 10,0.7,0.0,0.0); -fx-text-fill: red; -fx-font: 32pt 'Lao MN'");
