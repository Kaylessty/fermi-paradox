        ...
        for (int row = 0; row < currRoom.getRoom().length; row++) {
            for (int column = 0; column < currRoom.getRoom()[row].length; column++) {
                if (currRoom.getRoom()[row][column] != null) {
                    ...
                    try {
                        Image picture;
                        if(important instanceof Larry || important instanceof TreeBore || important instanceof Teeth) {
                            picture = new Image(imageURL, 200, 200, true, true);
                        } else if (important instanceof Item) {
...
//         Display the hatch if need be
    ...
    private void changeRoom(Door door) {
        ...
        if ((door.getLocked() && (inv.getValue() == null || (!(inv.getValue().getImageURL().equals("resources/images/TIMKEY.png")))))) {
            System.out.println("door locked");
            return;
        } else if(inv.getValue() != null && inv.getValue().getImageURL().equals("resources/images/TIMKEY.png")) {
...
//        }
    ...
    public void attack(Monster monster) {
        ...
        if (inv.getValue() != null) {
            ...
            if (distance <= range || (inv.getValue().getImageURL().equals("resources/images/SHADOWSWORD.png") &&
                    distance == 5)) {
                // attack monster
                if (carrying.getimageURL().equals("resources/images/PISTOL.png")
                        || carrying.getimageURL().equals("resources/images/RIFLE.png") ||
                        carrying.getimageURL().equals("resources/images/H_RIFLE.png")) {
...
//                    monster.setHasBeenAttacked(true);
        ...
        if (!foundMonster) {
            ...
            } else if(monster.getType().equals("Engi")) {
    ...
    private class MovementController implements EventHandler<KeyEvent> {
        ...
        public void handle(KeyEvent e) {
            ...
            if (e.getCode() == KeyCode.E) {
                if(inv.getValue().getType().equals("heal")) {
                    if(Player.getHealth().get() < Player.getMaxHealth() - inv.getValue().getDamage()) {
                        Player.setHealth(Player.getHealth().get() + inv.getValue().getDamage());
                        remove(inv.getValue());
                    } else {
                        Player.setHealth(Player.getMaxHealth());
                        remove(inv.getValue());
                    }
                } else if(inv.getValue().getImageURL().equals("resources/images/TEAFFHIDE.png")) {
                    Player.getHealth().set(Player.getHealth().get() + 500);
                    remove(inv.getValue());
                } else if(inv.getValue().getImageURL().equals("resources/images/AMMOBOX.png")) {
                    player1.addAmmo(10);
                    remove(inv.getValue());
                } else if(inv.getValue().getType().equals("Shield")) {
                    Player.setMaxHealth(Player.getMaxHealth() + inv.getValue().getDamage());
                    remove(inv.getValue());
                } else if(inv.getValue().getType().equals("charge")) {
                    Player.setGuncharged(inv.getValue().getDamage());
                    remove(inv.getValue());
                } else if(inv.getValue().getImageURL().equals("resources/images/SHADOWGAUNTLET.png")) {
                    gauntlet = true;
                    remove(inv.getValue());
                } else if(inv.getValue().getImageURL().equals("resources/images/BOREHEART.png")) {
                    heart = true;
                    remove(inv.getValue());
                }
